...
def quality_rank_score(quality: AudioQuality) -> tuple:
    """Return a comparable score tuple; larger means higher quality."""
    return (
        quality.average_bitrate or quality.bitrate or 0,
...
def pick_best_codec(
    ...
    """Pick the best available codec using config priority, then bitrate/depth."""
    if not qualities:
        return None
    priority = codec_priority or list(it(Config).download.codecPriority)
    best_per_codec: dict[str, AudioQuality] = {}
    for item in qualities:
        current = best_per_codec.get(item.codec)
        if current is None or quality_rank_score(item) > quality_rank_score(current):
            best_per_codec[item.codec] = item
    for codec in priority:
        if codec in best_per_codec:
            return codec
    return max(best_per_codec.values(), key=quality_rank_score).codec
